...
// API keys removed from frontend. All OxaPay calls go through
...
const HP = {
  ...
  // ── OxaPay — via Edge Functions (key never in browser) ──

  async createPaymentLink({ amount, currency = 'USDT', description, orderId }) {
    return this._call('create-payment', {
      amount,
      currency,
  ...
  async checkPayment(trackId) {
    return this._call('check-payment', { trackId });
  },

  async getOxaBalance() {
    return this._call('get-balance');
  },
